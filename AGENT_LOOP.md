...
| 領域 | 技術 | 再現性のための固定 |
|------|------|------|
| 言語 | Python | `.python-version` = **3.12**（uv が解決。CI も `uv python install` で同値） |
...
- テスト/フォーマットの**再現性を固定**（環境差による揺れを排除）:
  ...
  - `.python-version` = 3.12 を追加。`uv run` がローカルでも 3.12 を使い、CI（`uv python install` が
    `.python-version` を読む）と一致。版の単一の正は `.python-version`。
  - pytest は `uv.lock` で pin 済み（`make test` = `uv run pytest`）。→ 確認は Python 3.12.3 / ruff 0.15.17 で 58 緑。
