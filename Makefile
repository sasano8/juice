...
juice-init:
	@$(JUICE) bundle init $(BUNDLE_NAME) --clean

.PHONY: juice-bundle
## 内包物を vendoring し build コンテキスト（requirements/Dockerfile/entrypoint）を生成する（テスト用）
juice-bundle:
	@$(JUICE) bundle bundle $(BUNDLE_NAME)

.PHONY: juice-build
## docker build コマンドを生成・表示する（テスト用。実行は JUICE ... build --run）
juice-build:
	@$(JUICE) bundle build $(BUNDLE_NAME)

.PHONY: juice-run-ui
juice-run-ui:
	@$(JUICE) bundle run --bundle --env $(ENV_FILE) $(BUNDLE_NAME) ui

.PHONY: juice-run-api
juice-run-api:
	@$(JUICE) bundle run --bundle --env $(ENV_FILE) $(BUNDLE_NAME) api

.PHONY: juice-run-mcp_server
juice-run-mcp_server:
	@$(JUICE) bundle run --bundle --env $(ENV_FILE) $(BUNDLE_NAME) mcp_server
