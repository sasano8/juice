...
| E006 | manystore 抽出・統合 | 🚧抽出済/統合保留 | ストレージ抽象を別ライブラリ `manystore`（別repo）へ抽出済。juice への結線(adapter)は保留。下記「課題メモ: E006」参照 |
...
### 課題メモ: E006 — manystore（ストレージ抽象）の抽出と統合

> **背景/ゴール:** ストレージ抽象（`shoudou_storage` ＝ `FileStore` /local・nats・s3 バックエンド）を
> **別ライブラリ `manystore` として抽出**し、将来 juice の物理ストレージ差し込み点（`juice_storage` の
> `Storage`/`factory`）から利用する。juice はそれを**利用する側**になる。

**抽出済み（2026-06-20。別リポジトリ `/home/sasano8/projects/manystore`）:**
- `shoudou_storage/`（パッケージ）と `tests_storage/` を juice から **manystore へ移設**（`git mv` 不可＝別repo。
  manystore で `git init`＋初期コミット `2d28010`）。`uv run pytest` で **44 passed**。
- 実行依存（`redis>=5.0.0` / `nats-py>=2.0.0` / `aiobotocore>=2.0.0` / `httpx>=0.27.0`）は juice の
  dependency-group から **manystore の `[project.dependencies]` へ移動**。
- juice 側を掃除: pyproject から `shoudou-storage` group・`[tool.uv] default-groups`・`packages` の
  `shoudou_storage` を除去。`uv sync` で redis 等が .venv から外れる。`make check` 緑（221）。
- パッケージ名は `shoudou_storage`（import 名）、プロジェクト名は `manystore`（dir）。**rename は任意の follow-up**。

**制約（2026-06-20 ユーザー決定。維持）:** manystore（旧 shoudou_storage）は **pristine に保つ**。
juice 都合で `FileStore` に `walk`/`delete` 等を足す方針は**却下**。統合するなら **juice 側 src の adapter** に
橋渡しを閉じ込め、現状の `save/get/list/exists` だけから階層 Storage を構成するか doc-first で設計合意する。

**未統合（＝次にやること、保留中）:**
- juice の storage 差し込み点（`juice_storage`/`factory.py` の backend→Storage）から **manystore の FileStore を
  使う adapter** を juice src 側に置く（manystore は触らない）。`Config.backend`↔バックエンド対応、接続情報
  （redis/nats/s3 のエンドポイント・認証）の注入経路。
- 依存方向: juice → manystore（manystore を依存に追加するのは統合着手時）。

**注意:** 現状 juice は manystore を **import していない**（依存にも入れていない）＝juice 本体の挙動は不変。
`make check` 緑（221）。manystore 側は独立に `cd manystore && uv run pytest`（44）で検証できる。
...
**新規メモ（2026-06-20・要記憶）:** ストレージ抽象（`shoudou_storage`＋`tests_storage`）を**別ライブラリ
`manystore`（別repo `/home/sasano8/projects/manystore`）へ抽出済み**（44 passed）。juice は依存からも除去
（redis/nats/aiobotocore/httpx は manystore へ）。juice 本体は manystore を未 import＝挙動不変。将来 **juice の
storage を manystore に結合**（adapter は juice src 側・manystore は pristine）したい（→ **E006**）。
...
4. **E006 manystore 統合（保留中）** — 抽出は完了。残りは juice src 側 adapter で `manystore.FileStore` を
   factory に差し込む結線。**制約:** manystore を juice 都合で拡張しない（pristine 維持）。再開するなら
