...
| Q003 | ドキュメント整備 | 🚧作業中 | architecture.md に宣言系レイヤを反映（→「作業中」節） |
...
- **Q001（エラーメッセージの改善）を実装。** 宣言系コマンドの失敗を分かりやすくした。
  - CLI のエラー出力を `_fail(msg)` に集約（stderr＋exit 1）。manifest 失敗は `_fail_manifest` で
    **ファイルパス**を含める（`invalid manifest (juice.yaml): ...`）。apply は LockError も別経路で報告。
  - `_hint()` でよくある失敗に**次の一手**を添える（manifest 不在→パス確認、未対応 apiVersion→対応版、
    YAML 構文エラー→構文確認）。YAML エラーは PyYAML の **行情報**を含む（`parse_manifest` の wrap）。
  - tests/test_cli.py にエラー検証 3 ケース（パス＋ヒント、不在ファイル、YAML 行情報）。
- **結果:** `make check` 緑（計 124 ケース、Python 3.14.2 / ruff 0.15.17）。
- **関連 commit:** `53cf933`（Q002）に続き、本コミット（Q001）。
  ※ E2E 確認（会話 API）は今回 CLI 改善のため対象外（通常スキップ条件）。

### 作業中（In Progress / Next） — Q003: architecture.md に宣言系レイヤを反映

**概要（ゴール）:** `docs/architecture.md` は概念モデル（mcp_server / mcp_bundled / subagent / skill /
tool）と bundle/build/run のみで、**宣言系レイヤ（juice.yaml → lock → plan → apply、version）が
未記載**。ここを補い、「概念モデル」と「宣言的ワークスペース」の 2 軸が 1 枚で見えるようにする。

**タスク（チェックリスト）:**
- [ ] architecture.md に「宣言的ワークスペース（juice.yaml）」の節を追加: manifest が全レイヤを 1 ファイルで
      宣言し、`lock`（解決＋digest）→ `plan`（差分）→ `apply`（registries へ冪等 reconcile）で具現化する流れ。
- [ ] 概念モデル（再帰する mcp_server）と宣言系（juice.yaml の各レイヤ）の対応を明記（同じレイヤの別表現）。
- [ ] version（C004）/ 制約参照（C006）/ lock drift（C005）の位置づけを 1〜2 行で触れる。
- [ ] 相互リンクを整える（workspace.md / build.md / PROJECT.md）。SUPERSEDED な design.md との関係も明記。
- [ ] 図（mermaid）は既存の概念図に宣言系の小図を追加 or 文章で補足（壊さない範囲で）。

**完了条件:** architecture.md だけで「概念モデル＋宣言系ライフサイクル」の全体像が掴める。
docs のみの変更で `make check` 緑を維持。

**留意点:** docs のみ（コード非変更）。既存の概念図・包含制約の記述は保持し、追補に留める。
workspace.md（旧設計だが宣言系の出発点）と現行実装の差分に注意して書く。
