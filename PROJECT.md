...
- **python_packages を実レイヤ化（PyPI 様の registry を「同列の 1 registry」として横断対象に）。** これまで
...
`--build-deps`・compose `depends_on`）、**E002 remote mcp_server**（外部参照＝黒箱の全層対応）、
**python_packages の実レイヤ化**（layer=registry の横断対応）は実装済。ここから先は **(a) E001 の残り
