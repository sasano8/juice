...
- **C002 TODO（外部パッケージの digest 取得）を実装。** `juice.lock` の `digest` 欄（従来 null）を埋めた。
  - `src/core/digest.py` を新設（**副作用の境界**）。`npm_digest(package, version)` が npm registry の
    `dist.integrity`（SRI `sha512-...`）を返す。非 npm・取得失敗・オフラインは **None**（例外を投げない）。
    `fetch` を注入可能にし、ユニットテストは stub でネットワーク非依存。
  - `build_lock` / `write_lock` に `digest_resolver` を **依存注入**。未指定なら従来どおり `digest=None`
    （後方互換）。resolver を固定すれば digest 込みで冪等。`digest` は解決値（spec ではない）なので
    **manifestDigest には含めない**（drift 検出は spec 単独で決定的）。
  - CLI: `juice lock --resolve-digests`（既定 off、要ネットワーク）。`src/core/__init__.py` から re-export。
  - `tests/test_digest.py`（11 ケース）／`docs/workspace.md` の C002 メモ更新。
- **結果:** `make check` 緑（計 135 ケース、Python 3.14.2 / ruff 0.15.17）。`--resolve-digests` の
  オフライン null フォールバックも実機確認（`/tmp` で `juice lock` → `digest: null`）。
- **関連 commit:** `84fe8d5`（Q003）に続き、本コミット（C002 digest）。
  ※ E2E 確認（会話 API）は lock 機能のため対象外（通常スキップ条件）。実 npm 取得は要ネットワークで未実施。

### 作業中（In Progress / Next） — E004 第一歩: registry md の name とディレクトリ名一致の検証

**概要（ゴール）:** E004（OKF 適用）は OKF 仕様の調査が前提で大きいが、その中で**外部仕様に依存しない
核**＝「メタデータの `name` がファイル／ディレクトリ名に一致するか」の検証は先行実装できる。まずは
`juice` の registry（`registries/<ns>/<layer>/<name>/...` の md frontmatter）について、宣言された `name` と
ディレクトリ名の不一致を **検出して報告**する（自動修正はしない＝人間に委ねる、課題メモの要件 3）。

**タスク（チェックリスト）:**
- [ ] registry の md レイアウトと frontmatter の現状を調査（`registries/default` 配下。`name` キーの有無）。
- [ ] md frontmatter をパースして `name` を取り出す小関数（PyYAML。manifest.py の流儀に合わせ独立モジュール）。
- [ ] `name` ≠ ディレクトリ名を検出する検証を追加（不一致は**エラーとして列挙**、自動修正しない）。
      `juice manifest validate` ではなく registry 側の検証（apply/materialize と近い。重複を避けて寄せる）。
- [ ] CLI から呼べる入口を用意（例: `juice registry verify` か既存コマンドへの相乗り。要検討）。
- [ ] tests を追加（一致 OK／不一致で報告／name 欠落の扱い）。

**完了条件:** registry 内の md の `name` とディレクトリ名の不一致を検出・報告できる（自動修正なし）。
`make check` 緑。OKF の frontmatter スキーマ本体（要件 1 の OKF 準拠・要件 4 のインデックス）は次段階。

**留意点:** OKF の完全仕様はまだ未調査（課題メモの「前提」）。今回は OKF 非依存の **name=dir 検証のみ**に
絞り、OKF スキーマ／メタデータインデックス（source-of-truth の二重管理＝Open Question）は別サイクルに送る。
CLI list 系テストが実レジストリ（`registries/default`）に依存している点に注意（期待値の同期）。
