...
- [~] workflow のデプロイ成果物生成（E001 第二歩。`juice workflow build` が docker-compose を生成。生成のみ）
...
- [~] workflow（複数 instance の協調）— E001。宣言パイプライン（manifest/validate/apply＋plan・drift）＋デプロイ成果物生成（compose、生成のみ）は実装済。**実起動（`up`）・スケジューラ・k8s target・step 協調（順序/データ受け渡し）は未**
...
| ID | タスク | 状態 | 備考 |
|----|--------|------|------|
| E001 | workflow 実装 | 🚧一部完了 | 宣言パイプライン（`WorkflowSpec`／`apply._workflow`／validate／plan・drift）＋デプロイ成果物生成（`src/core/deploy.py`／`juice workflow build`／docker-compose、生成のみ）は実装済。残りは実起動（`up`）・スケジューラ・k8s target・step 協調 |
...
- **E001 第二歩（workflow のデプロイ成果物生成）を実装。** ユーザーとの設計対話で実行モデルを確定した：
  **juice は workflow を実行しない。宣言から実行基盤が食えるデプロイ成果物を生成し、常駐・協調・監視は外部基盤
  （docker compose / 将来 k8s＋ArgoCD）に委譲する**（`apply`＝registries 生成、`build`＝image 生成と同型）。
  ユーザー選択は **「生成のみ（compose を吐く）」**。
  - `src/core/deploy.py` を新設。`build_compose`（workflow→docker-compose v2 dict。各 step の mcp_bundled image を
    規約名 `juice/<name>[:version]` の長期常駐 service `restart: unless-stopped` に。`input`→環境変数、`schedule`→
    label）／`build_deployment`（target ディスパッチ。`_TARGETS` で pluggable）／`write_deployment`（`deploy/<name>/
    docker-compose.yml` へ冪等書き出し）／`find_workflow`。決定的。
  - CLI: `juice workflow build <name> [-f juice.yaml] [-o deploy] [--target compose]`。workflow 不在・未対応 target は
    rc 1＋ヒント。core re-export（`build_deployment` / `write_deployment`）。
  - tests: `tests/test_deploy.py`（8 ケース：構造／version 有無の image／ファイル名・ヘッダ／決定性／未対応 target／
    workflow 不在／冪等・パス／同一 bundle の連番 service）。
- **結果:** `make check` 緑（計 160 ケース、Python 3.14.2 / ruff 0.15.17）。tmp の juice.yaml で
  `juice workflow build morning-brief`→`deploy/morning-brief/docker-compose.yml` 生成、不在 workflow と未対応 target で
  rc 1 を実機確認。
- **docs:** architecture.md の workflow 節に「実行モデル（E001 第二歩）＝生成のみ・pluggable target・compose 詳細」を追記。
- **関連 commit:** 本コミット（E001 第二歩＝デプロイ成果物生成）。
  ※ E2E 確認（会話 API）は生成系機能のため対象外（通常スキップ条件）。

### 作業中（In Progress / Next） — E001 第三歩: k8s manifest target を追加（生成のみ。ArgoCD 連携を見据える）

**概要（ゴール）:** 第二歩で作った pluggable target の seam（`deploy.py` の `_TARGETS`）に **2 つめの target＝
Kubernetes manifest** を足し、`juice workflow build <name> --target k8s` で k8s に投入できる成果物を**生成のみ**で出す。
ユーザーが挙げた「k8s manifest を吐いて ArgoCD と連携」の路線。compose と同じく juice は生成に徹し、適用
（`kubectl apply` / ArgoCD）は外部に委譲する。

**着手前に決める設計事項:**
- 生成する kind：常駐 service は **Deployment**（`schedule` 付きは **CronJob**）に振り分けるか、まず Deployment のみか。
- 1 ファイル（multi-doc YAML）か、リソース毎ファイルか（ArgoCD は dir 単位で読めるので複数可）。
- image / env / label のマッピングを compose と揃える（規約 `juice/<name>[:version]`、`input`→env、`schedule`→
  CronJob schedule か annotation）。

**タスク（チェックリスト）:**
- [ ] `deploy.py` に `build_k8s`（workflow→k8s manifest dict/複数 doc）を追加し `_TARGETS["k8s"]` に登録。
- [ ] `schedule` 有無で Deployment / CronJob を振り分ける（まずは最小で）。出力は決定的・冪等に。
- [ ] CLI は既存の `--target` で透過（新規フラグ不要）。出力ファイル名は target 側で決める（例: `manifests.yaml`）。
- [ ] tests（k8s 生成の構造／schedule での CronJob 化／決定性／既存 compose を壊さない）。
- [ ] docs（architecture.md の workflow 節に k8s target と ArgoCD 連携の位置づけを追記）。

**完了条件:** `juice workflow build <name> --target k8s` が決定的な k8s manifest を生成でき、compose target は不変。
`make check` 緑。

**留意点:** **生成のみ**を維持（`kubectl apply`・実起動・スケジューラ稼働は別サイクル）。k8s スキーマは最小限に
留め、過剰なフィールドは入れない（YAGNI）。step 協調（順序・データ受け渡し）はまだ扱わない（compose/k8s とも
独立 service/Deployment のまま。協調セマンティクスは別サイクルで設計）。
