...
- **Q003（ドキュメント整備）を実装。** `docs/architecture.md` に宣言系レイヤを反映した。
  - 冒頭に **2 軸**（概念モデル／宣言的ワークスペース）の見取り図を追加。
  - 新節「**宣言的ワークスペース（juice.yaml）**」を bundle/build/run と workflow の間に追加:
    `juice.yaml → lock → plan → apply → registries/` のライフサイクル表（コマンド↔実装 C001〜C006）と、
    依存順 reconcile・manifest.py の層分離に言及。
  - 「**概念モデルと宣言の対応**」表で、概念レイヤと `juice.yaml` の各キーの 1:1 対応を明記。
  - 「**version / 制約 / drift**」で C004（SemVer）/ C006（制約参照）/ C005（lock drift）の位置づけを整理。
  - 相互リンク（build.md / workspace.md）と SUPERSEDED な workspace.md との関係を明記。
- **結果:** docs のみの変更。`make check` 緑（計 124 ケース、Python 3.14.2 / ruff 0.15.17）。
- **関連 commit:** `4c3bab6`（Q001）に続き、本コミット（Q003）。
  ※ E2E 確認（会話 API）は docs のみのため対象外（通常スキップ条件）。

### 作業中（In Progress / Next） — C002 TODO: lock の外部パッケージ digest 取得

**概要（ゴール）:** `juice.lock` の `digest` 欄は現状 `null`（C002 の積み残し）。mcp_server が参照する
外部パッケージ（npm の `@example/mcp-weather` 等）の**内容同一性を pin** できるよう、digest を解決して
lock に記録する。これにより「同じ spec＋lock なら同じ取得物」が digest で担保され、設計原則
「生成物を焼かず spec から再生成、整合性は digest で担保」（C005 の drift 検出と同系）が完成する。

**タスク（チェックリスト）:**
- [ ] digest の取得元・形式を決める（npm registry の `dist.integrity`=SRI / OCI manifest digest）。
      まずは **npm を対象**に絞る（package が `@scope/name` 形式のもの）。version 未指定時の解決規則も決める。
- [ ] `src/core/lock.py` に digest 解決を追加（ネットワーク I/O は注入可能にしてテストで stub。
      取得失敗・オフライン時は `null` のまま警告 → 既存挙動を壊さない後方互換）。
- [ ] `manifestDigest` に digest を含めるか整理（lock の再現性入力に含める。drift 検出との二重計上に注意）。
- [ ] tests を追加（stub した fetcher で digest が記録される／失敗時 null フォールバック／冪等）。
- [ ] workspace.md / architecture.md の該当メモ（digest 欄 null）を更新。

**完了条件:** npm package を参照する mcp_server について `juice lock` が digest（SRI）を記録し、再 lock が
冪等。ネットワーク不可時は `null` フォールバックで従来どおり動く。`make check` 緑。

**留意点:** ネットワーク依存はテストを不安定にする。fetcher を **依存注入**にしてユニットテストは stub で
回す（実ネットワークは E2E/手動のみ）。digest 形式は npm の `integrity`（`sha512-...`）をそのまま採用すると
独自正規化を避けられる（設計原則「標準フォーマット準拠」）。OCI は必要になってから（YAGNI）。
