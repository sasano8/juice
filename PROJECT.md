...
### 未実装・課題
- [~] `juice.yaml` による宣言的ワークスペース（workspace.md 参照）※parser/lock/apply 済み。lock 連携と plan は未
- [ ] `juice plan`（差分表示）＋ apply の lock drift 検出（宣言ライフサイクルの統合）
- [ ] バージョニングと依存解決（SemVer）
...
| C005 | 宣言ライフサイクルの統合 | 🚧作業中 | `juice plan`（差分）＋ apply の lock drift 検出（→「作業中」節） |
| C004 | バージョニング機構 | 未着手 | SemVer 対応（設計が必要なため C005 の後） |
...
- **C003（`juice apply`）を実装。** `src/core/apply.py` を新設。
  - manifest の各レイヤを依存順（mcp_server → skill / subagent → mcp_bundled → instance）で
    現行 registry レイアウト（build.md）へ materialize し、`RegistryArray.write` で反映。
    materialize 先: tool→`tools/<name>/index.md`（command/args/env。command 文字列は先頭=コマンド/
    残り=args に分解）、subagent→`subagents/<name>/index.md`（frontmatter＋prompt）、
    skill→`SKILL.md`、mcp_bundled→`bundle.yml`、instance→`index.yml`。
  - **冪等**: 既存と同一内容なら skip（再 apply で written/pruned とも空＝no-op）。宣言にない既存
    パッケージは **prune**（ディレクトリごと削除）。`prune=False` / `dry_run=True` 対応。
  - CLI `juice apply [-f juice.yaml] [-n ns] [--no-prune] [--dry-run]` を追加。
    facade に `Juice.apply`、core に `apply_manifest` を公開。tests/test_apply.py（7）＋ CLI 2 ケース。
  - docs/workspace.md「再現性のモデル」に C003 の実装メモを追記。
- **結果:** `make check` 緑（計 76 ケース、Python 3.14.2 / ruff 0.15.17）。
- **関連 commit:** `9cdd16e`（C002）に続き、本コミット（C003）。
  ※ E2E 確認（会話 API）は今回 LLM/コンテナ不要のファイル materialize 機能のため対象外（通常スキップ条件）。

### 作業中（In Progress / Next） — C005: 宣言ライフサイクルの統合

> 次は backlog 順では C004（バージョニング）だが、SemVer は設計を要するため後回しにし、
> C001–C003 で揃った lock / apply を**繋ぐ** C005 を先に行う（lock → plan → apply の一筆書きを完成させる）。

**概要（ゴール）:** `juice.lock` と `juice apply` を連携させ、宣言ライフサイクル（lock → plan → apply）を
完成させる。(1) `juice plan` を差分表示の第一級コマンドにする（現状 apply の `--dry-run` を整理して昇格）。
(2) apply 時に `juice.lock` の `manifestDigest` を照合し、spec と lock の **drift を検出**して警告/エラーにする。

**タスク（チェックリスト）:**
- [ ] `juice plan [-f juice.yaml]`: apply を書き込まず実行し、written/pruned を差分として表示
      （内部は `apply_manifest(..., dry_run=True)` を再利用）。CLI と facade を整える。
- [ ] drift 検出: apply/plan 時に `juice.lock` があれば読み、`build_lock(manifest).manifest_digest` と
      lock の `manifestDigest` を比較。不一致なら「`juice lock` を実行して」と促す（既定は警告、`--frozen` でエラー）。
- [ ] lock 欠如時の扱いを決める（警告のみで続行 / `--require-lock` で要求）。
- [ ] tests: plan が書き込まないこと、drift 検出（lock 更新後は一致）。tmp レジストリ＋tmp lock を使う。
- [ ] docs/workspace.md「ライフサイクル」を plan/apply の実装に追従。

**完了条件:** `juice plan` が差分を表示し（無変更なら空）、`juice apply` は lock と manifest の drift を
検出して案内する。`make check` 緑（新規テスト含む）。

**留意点:** registries/ は「apply の出力」（生成物）という workspace.md の設計と、現行の
「registries/ を手書き一次情報として list する」運用（build.md / 既存テスト）が衝突しうる。
apply/plan のテストは必ず tmp レジストリで行い、実レジストリ（registries/default）を壊さないこと。
