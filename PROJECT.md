...
- **E001 step 協調の第一歩：workflow/compose に宣言順の直列 `depends_on` を実装。** step が独立 service の
...
`--build-deps`・compose `depends_on`）は一通り実装済。ここから先は **(a) E001 の残り（実起動・スケジューラ
稼働・完了待ち/DAG）＝juice の「実行しない」原則に触れる大物**、または **(b) 別バックログ** に分岐する。
**着手時にユーザーへ優先度を確認**してから 1 つ選ぶ。

**候補（上から推奨順・要確認）:**
1. **E002 remote mcp_server 対応** — manifest は `from: mcp_server:<name>` を既にパースするが、remote（外部参照＝
   `package`/`version` で黒箱参照、bundled しない）の解決・materialize が未。`SUPPORTED_BIND_KINDS` は `mcp_server`
   のみ。最下層の参照を固めると上位が安定する。
2. **`python_packages` を実レイヤ化（最小）** — 今は構造のみ（`namespaces/default/python_packages/README.md`、
...
3. **E003 skill ライブラリ** — 再利用可能な skill 集。
4. **E001 残り（大物）** — 実起動（`up`/`kubectl apply`）・スケジューラ稼働・step 完了待ち/データ受け渡し/DAG。
