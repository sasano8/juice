...
- [~] workflow / schedule のデプロイ成果物生成（E001。workflow=常駐〔compose service／k8s Deployment〕、schedule=定期実行〔k8s CronJob／compose one-shot〕。生成のみ）
...
| ID | タスク | 状態 | 備考 |
|----|--------|------|------|
| E001 | workflow 実装 | 🚧一部完了 | 定義(workflow=常駐)とトリガ(schedule=定期実行)を分離。宣言パイプライン＋デプロイ成果物生成（`src/core/deploy.py`／`juice workflow build`・`juice schedule build`／compose＋k8s、生成のみ）は実装済。残りは schedule の registry レイヤ化・実起動・スケジューラ稼働・step 協調 |
...
- **workflow と schedule の概念分離をリファクタ。** ユーザーとの設計対話で「`schedule` は workflow の持ち物では
...
### 作業中（In Progress / Next） — E001: schedule を registry レイヤに昇格（apply materialize＋index＋verify）

**概要（ゴール）:** 現状 schedule は manifest＋deploy 生成のみで、workflow と違い **registry レイヤではない**
（`apply` で materialize されず、`juice schedule list`・index・OKF 検査の対象外）。juice の「全レイヤを
宣言→apply で materialize、index/verify が横断」という一貫性に合わせ、schedule を 1 レイヤに昇格する。

**タスク（チェックリスト）:**
- [ ] `config.py`: `LAYERS`/`ENTRY_FILES`/`ALL_ORDER` に `schedule`（`schedules/<name>/index.md`）を追加。
- [ ] `apply.py`: `_schedule` materialize（frontmatter kind/name/type/schedule/steps）＋ `_LAYER_ORDER` に追加。
      OKF `type: schedule` を付与（.md concept doc なので verify_okf 対象）。
- [ ] `juice schedule list` が LAYERS 経由で自動で増えることを確認。index（ALL_ORDER）にも自動収録。
- [ ] registries/default に `schedules/morning-brief/index.md`（cron＋steps）を追加（schedule の実例を復活）。
- [ ] tests（apply で schedule materialize＋冪等／OKF type／manifest_digest が schedules を含む drift）。
      CLI list 系テストは実レジストリ依存＝期待値を同期。
- [ ] docs（architecture.md の宣言ライフサイクルに schedule レイヤを反映）。

**完了条件:** `juice.yaml` の `schedules:` を `juice apply` で `schedules/<name>/index.md` に冪等 materialize でき、
`juice schedule list`／index／verify が schedule を横断対象にする。`make check` 緑。

**留意点:** deploy 生成（`juice schedule build`）は manifest から読むので本変更と独立。registries/default に
schedule を足すと CLI list 系テストの期待値同期が要る点に注意（既存 workflow 追加時と同様）。step 協調（順序・
データ受け渡し）と実起動は引き続き別サイクル（YAGNI）。
