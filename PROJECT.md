...
**候補（上から推奨順・要確認）:**
1. **E006 shoudou_storage 統合** — juice の storage 差し込み点（factory）から shoudou_storage の FileStore を
   使えるようにする。最終的に別ライブラリへ抽出。詳細は「課題メモ: E006」。
2. **E003 skill ライブラリ** — 再利用可能な skill 集。
3. **python_packages の中身（follow-up）** — 実レイヤ化は済。次は wheel/sdist の実格納と PEP 503 風の
