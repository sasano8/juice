...
- [~] workflow を宣言パイプラインに載せる（E001 第一歩。manifest/validate/apply materialize＋plan・drift 対応。実行は未）
...
- [~] workflow（複数 instance の協調）— E001。宣言パイプライン（manifest/validate/apply materialize＋plan・drift）は実装済。**実行・協調（複数 instance の起動順・データ受け渡し・スケジューラ）は未**
...
| ID | タスク | 状態 | 備考 |
|----|--------|------|------|
| E001 | workflow 実装 | 🚧一部完了 | 宣言パイプライン（`WorkflowSpec`／`apply._workflow`／validate／plan・drift）は実装済。残りは実行・協調（起動順・データ受け渡し・スケジューラ） |
...
- **E001 第一歩（workflow を宣言パイプラインに載せる）を実装。** `workflows/<name>/index.md` は手書きで
  存在していたが、宣言パイプライン（manifest → validate → apply）に未接続だった。これを instance と同型に接続した。
  - `manifest.py`: `WorkflowStep`（mcp_bundled＋input）／`WorkflowSpec`（name / steps / 任意 schedule / version）を
    追加。`Manifest.workflows` ＋ `_parse_workflow` / `_parse_workflow_step`。validate で name 重複と
    `steps[].mcp_bundled` の参照解決（未定義 bundle を弾く）を確認。
  - `apply.py`: `_workflow` materialize（`workflows/<name>/index.md`、frontmatter `kind/name/type/schedule/steps`）。
    `_LAYER_ORDER` 末尾に workflow を追加（最上位の協調レイヤ）。生成 md は OKF 適合（`type: workflow`）。
  - **plan/drift は自動対応:** `manifest_digest` が `dataclasses.asdict(manifest)` 由来のため、`workflows` 追加で
    drift 検出が自動でカバーされる。lock の per-workflow pin は不要と判断（spec のみ。必要になってから）。
  - tests: `test_manifest.py` に workflow 5 ケース（パース／step 必須／未定義参照／input 型／重複）、
    `test_apply.py` に materialize＋冪等 2 ケース。
- **結果:** `make check` 緑（計 152 ケース、Python 3.14.2 / ruff 0.15.17）。tmp registry で
  `juice.yaml`→`apply`→`workflows/morning-brief/index.md` 生成、`verify_okf`＝[]（適合）、index 収録を実機確認。
- **docs:** architecture.md の宣言ライフサイクル表・概念対応表・workflow 節に「宣言→materialize（実行は未）」を追記。
- **関連 commit:** 本コミット（E001 第一歩＝workflow 宣言パイプライン）。
  ※ E2E 確認（会話 API）は宣言系機能のため対象外（通常スキップ条件）。

### 作業中（In Progress / Next） — E001 第二歩: workflow 実行モデルの設計＋最小の「実行計画」解決

**概要（ゴール）:** workflow の核心＝**複数 instance の協調実行**へ進む。ただし実行系は設計判断（起動順・
データ受け渡し・スケジューラの有無・instance ライフサイクルとの関係）が多いので、本サイクルは
**①実行モデルの設計を docs に明文化し、②「実行計画（plan）」を解決する最小実装**（実際の起動はまだしない）に絞る。
workflow の各 step（mcp_bundled 参照）を、対応する instance／起動パラメータへ解決した**実行計画 dict** を返す
ところまで。実際のコンテナ起動・協調ランタイムはさらに次段階。

**着手前に決める設計事項（要ユーザー確認の可能性）:**
- step → instance の対応：既存 instance を名前参照するか、workflow が ephemeral に instance 化するか。
- step 間のデータ受け渡し（`input` と前段出力の関係）。直列のみか、依存 DAG か。
- `schedule` の実行主体（juice 内スケジューラか、外部 cron へ委譲か）。

**タスク（チェックリスト）:**
- [ ] 実行モデルを docs（architecture.md の workflow 節）に明文化（上記 3 点の決定を含む）。
- [ ] `WorkflowStep` を instance／起動パラメータへ解決する `resolve_execution(manifest, name)` を実装（起動はしない）。
- [ ] 参照整合（step の mcp_bundled に対応する instance/bundle があるか）を実行計画解決時にも確認。
- [ ] CLI から実行計画を確認できる surface（例: `juice workflow plan <name>`）を検討・最小実装。
- [ ] tests（実行計画の解決／参照不整合エラー／dry な計画が決定的）。

**完了条件:** workflow の実行モデルが docs で定義され、`juice.yaml` の workflow から決定的な「実行計画」を
解決できる（実起動はまだ）。`make check` 緑。

**留意点:** 実行系は YAGNI を効かせて**計画解決まで**に留める（実起動・スケジューラ・並行制御は別サイクル）。
設計事項に未決があれば、決め打ちせず作業開始時に整理してから実装する（必要ならユーザーに確認）。
manifest_digest は既に workflow を含むため、実行計画は spec/lock と整合する形で組む。
