...
- [~] workflow のデプロイ成果物生成（E001 第二〜三歩。`juice workflow build` が docker-compose／k8s manifest を生成。生成のみ）
...
- [~] workflow（複数 instance の協調）— E001。宣言パイプライン（manifest/validate/apply＋plan・drift）＋デプロイ成果物生成（compose／k8s manifest、生成のみ）は実装済。**実起動（`up`／`kubectl apply`）・スケジューラ稼働・step 協調（順序/データ受け渡し）は未**
...
| ID | タスク | 状態 | 備考 |
|----|--------|------|------|
| E001 | workflow 実装 | 🚧一部完了 | 宣言パイプライン（`WorkflowSpec`／`apply._workflow`／validate／plan・drift）＋デプロイ成果物生成（`src/core/deploy.py`／`juice workflow build`／compose＋k8s manifest、生成のみ）は実装済。残りは実起動・スケジューラ稼働・step 協調 |
...
- **E001 第三歩（k8s manifest target）を実装。** 第二歩で作った pluggable target の seam（`deploy.py` の
  `_TARGETS`）に 2 つめの target＝Kubernetes を追加。`juice workflow build <name> --target k8s` で生成のみ。
  ユーザーが挙げた「k8s manifest を吐いて ArgoCD と連携」の路線。
  - `deploy.py`: `build_k8s`（workflow→k8s リソースの list）を追加。`schedule` 有なら各 step を **CronJob**、
    無なら **Deployment**（replicas:1 の長期常駐）に振り分け。image/env/label は compose target と揃える
    （規約 `juice/<name>[:version]`、`input`→env、metadata.name は `<workflow>-<svc>`）。`_dump` を multi-doc
    対応に一般化し、`_TARGETS` を `(builder, filename, multi_doc)` 形に拡張（`k8s`→`manifests.yaml`、multi-doc）。
  - CLI は既存の `--target` で透過（新規フラグ不要）。
  - tests: `test_deploy.py` に k8s 5 ケース（CronJob 化／Deployment 化／multi-doc ファイル／決定性／write）。
- **結果:** `make check` 緑（計 165 ケース、Python 3.14.2 / ruff 0.15.17）。tmp の juice.yaml で
  scheduled→CronJob、no-schedule の 2 step→Deployment×2（multi-doc）を `--target k8s` で生成・実機確認。
- **docs:** architecture.md の workflow 節を compose／k8s の 2 target 構成に更新。
- **関連 commit:** 本コミット（E001 第三歩＝k8s manifest target）。
  ※ E2E 確認（会話 API）は生成系機能のため対象外（通常スキップ条件）。

### 作業中（In Progress / Next） — E001 第四歩: step 協調（直列実行順序）を表現する（compose depends_on、生成のみ）

**概要（ゴール）:** ここまで step は独立 service / Deployment のままで「協調」が無い（＝ただのサービス群）。
workflow の定義的機能＝**step 間の実行順序**を、生成成果物に表現する。本サイクルは compose target で
**直列順序（宣言順に前段へ `depends_on`）** を最小実装する（生成のみ）。

**着手前に決める設計事項（未決なら作業開始時に整理。必要ならユーザー確認）:**
- 順序モデル：**直列のみ**（宣言順）か、step に依存を宣言させる **DAG** か。→ まず直列を既定とする（YAGNI）。
- compose の `depends_on` は「起動順」であって「完了待ち」ではない。pipeline 的な完了待ち（前段 Job 完了→後段）が
  要るなら compose では不足で、k8s 側は Argo Workflows 等が必要。**まず compose の起動順序のみ**に絞る。
- 同一 bundle の連番 service（`-2`）と depends_on の関係を矛盾なく。

**タスク（チェックリスト）:**
- [ ] `build_compose` に直列 `depends_on`（各 service が直前の service に依存）を付与。単一 step なら無し。
- [ ] k8s target の順序表現は**本サイクルでは保留**（独立 Deployment のまま）。docs に「k8s の順序は別途（Argo 等）」と明記。
- [ ] tests（depends_on の連鎖／単一 step では付かない／決定性／k8s は不変）。
- [ ] docs（architecture.md に直列協調の仕様と compose/ k8s の差を追記）。

**完了条件:** compose 生成物が宣言順の直列 `depends_on` を持ち、決定的。k8s target は不変。`make check` 緑。

**留意点:** **生成のみ**を維持。`depends_on` は起動順序にすぎない点を docs で明示し、pipeline 的完了待ち・DAG・
実起動・スケジューラ稼働は別サイクル（YAGNI）。協調の本格設計（完了待ち/データ受け渡し）は規模が大きいので
段階を分ける。
