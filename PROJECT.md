...
- [x] 資産カタログ（横断ビュー。`juice catalog`。index 集約を標準スキーマ＝identity＋OKF type＋推奨フィールドへ射影。type/tag 絞り込み）
...
| ID | タスク | 状態 | 備考 |
|----|--------|------|------|
| E001 | workflow 実装 | 🚧一部完了 | 定義(workflow=常駐)とトリガ(schedule=定期実行)を分離。宣言パイプライン＋デプロイ成果物生成（compose＋k8s）＋schedule の registry レイヤ昇格＋依存閉包解決（`deploy.dependency_closure`）＋依存の実ビルド起動（`build --build-deps`）は実装済。残りは実起動(`up`/`kubectl apply`)・スケジューラ稼働・step 協調 |
...
**任意の follow-up:** ①OKF 推奨フィールドの**横断ビュー**は `juice catalog`（`src/core/catalog.py`）で実装済
...
- **資産カタログ（横断ビュー `juice catalog`）を実装。** E004 follow-up ①（OKF 推奨フィールドの横断ビュー）の完成形。
  前サイクルの作業ツリーに未コミットで残っていた catalog 実装を仕上げた（PROJECT.md の「作業中」には
  E001 depends_on と書かれていたが、ツリーの実体は catalog だった＝乖離を解消。実装が一貫し概ね完成していたため破棄せず完成させた）。
  - `src/core/catalog.py`（新規）: `build_catalog` が index 集約（`build_index`）を土台に各資産を**標準スキーマ**
    （identity＝`name`/`layer` ＋ OKF `type` ＋ OKF 推奨フィールド `title`/`description`/`tags`/`resource`/`timestamp`）へ射影。
    推奨フィールドは任意で欠落は省略（verify を壊さない）。`filter_catalog` が type/tag で絞り込む。新レイヤではなく**ビュー**。
  - `core/__init__.py`: `Juice.catalog(type_, tag)` を公開（`build_catalog`＋`filter_catalog`）。
  - CLI: `juice catalog [--type <concept type>] [--tag <tag>]`。出力は `layer/name (type) [tags] — description`。
    epilog 例は実在 type に修正（`--type mcp-server`。tool の OKF concept type は `mcp-server` で `tool` ではない）。
  - registries: weather tool / report-weather skill に `description`/`tags` を追記（catalog の射影確認用の実例）。
  - tests: `test_catalog.py`（射影・任意フィールド・type/tag 絞り込み 6 ケース）＋ `test_cli.py` に CLI 一覧/tag 絞り込み/help 例 3 ケース。
  - docs: architecture.md の registry 健全性節に「資産カタログ（横断ビュー）」を追記。
  - lint: catalog.py の docstring 2 行が E501（全角で 100 超）だったので折り返して解消。
- **結果:** `make check` 緑（計 184 ケース）。実機で `juice catalog`／`--tag weather`／`--type mcp-server`／`-h` を確認。
- **関連 commit:** 本コミット（資産カタログ）。
...
### 作業中（In Progress / Next） — E001: step 協調（直列起動順）を生成物に表現する（compose depends_on）

**概要（ゴール）:** ここまで step は独立 service / Deployment のままで「協調」が無い。workflow（常駐サービス群）の
**step 間の起動順**を生成物に表現する。本サイクルは compose target で **宣言順の直列 `depends_on`**（各 service が
直前の service に依存）を最小実装する（生成のみ）。これにより「複数 instance の協調」の第一歩＝起動順序が出る。

**着手前に整理（必要ならユーザー確認）:**
- workflow（常駐）の `depends_on` は **起動順**（compose の意味）であって「完了待ち」ではない。pipeline 的な
  完了待ち・データ受け渡しは別（compose では不足、k8s は Argo 等）。本サイクルは**起動順のみ**。
- 順序モデルは **直列（宣言順）** を既定。DAG は後回し（YAGNI）。
- k8s（Deployment）には depends_on 相当が無い → **k8s は本サイクル不変**（順序は Argo 等で別途、と docs 明記）。
- schedule（ジョブ）への順序適用は別論点（pipeline）。本サイクルは workflow/compose に限定。

**タスク（チェックリスト）:**
- [ ] `build_compose`（workflow）に直列 `depends_on` を付与（2 番目以降の service が直前に依存）。単一 step は無し。
- [ ] 同一 bundle の連番 service（`-2`）でも順序が決定的・矛盾なく。
- [ ] tests（depends_on の連鎖／単一 step では付かない／決定性／k8s・schedule は不変）。
- [ ] docs（architecture.md に「直列起動順（compose depends_on）」と compose/k8s の差を追記）。

**完了条件:** workflow の compose 生成物が宣言順の直列 `depends_on` を持ち決定的。k8s・schedule は不変。`make check` 緑。

**留意点:** **生成のみ**。`depends_on`＝起動順にすぎない点を docs で明示。完了待ち・データ受け渡し・DAG・実起動・
E005 triggers は別サイクル（YAGNI）。代替候補：E002（remote mcp_server）/ E003（skill ライブラリ）へ移ってもよい。
着手時に優先度を確認する。
