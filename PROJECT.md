...
- [~] バージョニングと依存解決（SemVer）※version 宣言・検証・lock 記録は C004 で実装。制約参照/依存解決は未
- [ ] version 制約参照（`from: name@>=1.0`）と依存解決（範囲マッチ・複数版共存）
...
| C006 | version 制約参照・依存解決 | 🚧作業中 | `from: name@<制約>` の解決（→「作業中」節） |
...
- **C004（バージョニングの足場）を実装。** SemVer を導入。
  - **`src/core/semver.py`** を新設（外部依存なし）: `Version` / `parse_version`（不正は `SemverError`）/
    SemVer 優先順位での比較（prerelease 付きは無印より小さい）/ `satisfies`（`==`/`>=`/`<=`/`>`/`<`）。
  - manifest の各パッケージ Spec（mcp_server / subagent / skill / mcp_bundled）に任意 `version` を追加。
    不正な SemVer は `ManifestError`（YAML が float 化する `1.2` 等も不正として弾く）。
  - lock の `LockedServer` に `version` を記録（`manifestDigest` は `asdict(manifest)` 経由で version も反映）。
  - core に `Version / SemverError / parse_version / is_valid / satisfies` を公開。
    tests/test_semver.py（22）＋ manifest 2＋lock 2 ケース。
  - docs/workspace.md に C004 の実装メモを追記。
- **結果:** `make check` 緑（計 110 ケース、Python 3.14.2 / ruff 0.15.17）。
- **関連 commit:** `26abf27`（C005）に続き、本コミット（C004）。
  ※ E2E 確認（会話 API）は今回ライブラリ/検証機能のため対象外（通常スキップ条件）。

### 作業中（In Progress / Next） — C006: version 制約参照・依存解決

**概要（ゴール）:** C004 で作った `satisfies` を使い、参照に**バージョン制約**を持たせて検証する。
`from: mcp_server:weather@>=1.0.0` のように tool 束縛へ制約を書けるようにし、参照先 mcp_server の
宣言 `version` が制約を満たすかを validate で確認する。範囲マッチや複数版共存などの本格的な依存解決は
まだ作らない（YAGNI。単一宣言内の制約充足チェックに留める）。

**タスク（チェックリスト）:**
- [ ] `ToolBinding` の `from` パーサを拡張: `mcp_server:<name>@<制約>` を `from_name` ＋ `constraint`
      （任意）に分解する。`@` が無ければ従来どおり（後方互換）。
- [ ] 検証: 制約があるとき、参照先 mcp_server の `version` が `satisfies` を満たすか確認。
      version 未宣言なのに制約があれば `ManifestError`（解決不能）。
- [ ] lock/apply への影響を最小化（制約は manifest 側の検証で閉じる。lock 形は据え置きでも可）。
- [ ] tests: 制約充足・不充足（ManifestError）・version 未宣言・`@` 無しの後方互換。
- [ ] docs 更新（workspace.md のバージョン項を制約参照に追従）。

**完了条件:** `from: name@<制約>` を validate でき、不充足・解決不能は明確なエラー。`@` 無しは従来通り。
`make check` 緑（新規テスト含む）。

**留意点:** これも依存解決へ拡張しやすいが、**単一 manifest 内の制約充足**までで区切る。
複数版共存・レジストリ横断の解決は必要になってから別タスクにする。
