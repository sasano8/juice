...
| C004 | バージョニング機構 | 🚧作業中 | SemVer 対応（→「作業中」節） |
...
- **C005（宣言ライフサイクルの統合）を実装。** lock ↔ apply を繋ぎ、lock → plan → apply を完成。
  - **`juice plan`** を追加（apply の dry-run 昇格。書き込まず written/pruned を差分表示）。
    facade `Juice.plan`、CLI `juice plan`。内部は `apply(..., dry_run=True)` を再利用。
  - **drift 検出**: `lock_status(manifest, lock_path)`（`src/core/lock.py`）を追加し、`juice.lock` の
    `manifestDigest` と manifest を照合。`juice apply`/`plan` は lock があれば drift を検査し、
    既定は**警告**（結果 `warning`）、`--frozen` で `LockError`、`--require-lock` で lock 不在をエラー。
  - core に `LockError / lock_status` を公開。tests/test_plan.py（7）＋ CLI 2 ケース。
  - docs/workspace.md に C005 の実装メモを追記。
- **結果:** `make check` 緑（計 84 ケース、Python 3.14.2 / ruff 0.15.17）。
- **関連 commit:** `dbd4d88`（E004 課題追加）に続き、本コミット（C005）。
  ※ E2E 確認（会話 API）は今回 LLM/コンテナ不要のファイル/メタ機能のため対象外（通常スキップ条件）。

### 作業中（In Progress / Next） — C004: バージョニングと依存解決（SemVer）

**概要（ゴール）:** パッケージ（mcp_server / subagent / skill / mcp_bundled）と instance に **SemVer**
バージョンを導入し、参照時のバージョン制約解決と lock への記録の土台を作る。まずは
**(1) version フィールドの導入と検証**（妥当な SemVer か）から始め、解決アルゴリズムは最小に留める（YAGNI）。

**タスク（チェックリスト）:**
- [ ] SemVer のパース/比較を行う小さなユーティリティ（`src/core/semver.py`）。外部依存を足さず自前実装
      （`major.minor.patch` ＋任意 prerelease。範囲は当面 `==`/`>=` 程度）。まずは**検証**を最優先。
- [ ] manifest スキーマに `version`（任意）を追加（C001 の各 Spec）。不正な SemVer は `ManifestError`。
- [ ] lock に解決済み version を記録（`LockedServer` 等に `version` を追加）。`manifestDigest` の入力にも含める。
- [ ] 参照のバージョン制約（例: `from: mcp_server:weather@>=1.0`）をどう表すか設計（最小実装は固定版のみでも可）。
- [ ] tests/test_semver.py（パース・比較・不正値）＋ manifest/lock の version 反映テスト。
- [ ] docs 更新（workspace.md「未決の論点」のバージョン項を実装に追従）。

**完了条件:** version 付き manifest を validate / lock でき、不正な SemVer は明確なエラー。
`make check` 緑（新規テスト含む）。

**留意点:** スコープが膨らみやすい領域。まず「version の宣言・検証・lock 記録」までに区切り、
依存解決（範囲マッチや複数版共存）は別タスクに切り出す（必要になるまで作らない）。
