...
| Q002 | CLI ヘルプの充実 | 🚧作業中 | 各コマンドに例を追加（→「作業中」節） |
...
- **C006（version 制約参照）を実装。** tool 束縛の `from` にバージョン制約を持たせられるようにした。
  - `ToolBinding.from` パーサを拡張: `mcp_server:<name>@<制約>` を `from_name`＋`constraint` に分解
    （`@` 無しは従来どおり＝後方互換、`constraint=None`）。空制約 `name@` はエラー。
  - validate（`_check_constraint`）: 制約があるとき参照先 mcp_server の宣言 `version` が `satisfies` を
    満たすか確認。不充足・version 未宣言・制約不正は `ManifestError`。
  - lock/apply は `from_name` を使うため変更なし（制約は manifest 検証で閉じる）。
    tests/test_manifest.py に制約 6 ケース追加。
  - docs/workspace.md に C006 の実装メモを追記。
- **結果:** `make check` 緑（計 116 ケース、Python 3.14.2 / ruff 0.15.17）。
- **関連 commit:** `99ccdd5`（C004）に続き、本コミット（C006）。
  ※ E2E 確認（会話 API）は今回 manifest 検証機能のため対象外（通常スキップ条件）。

### 作業中（In Progress / Next） — Q002: CLI ヘルプの充実

> 宣言系コア（C001–C006）が一通り揃い、CLI コマンドが増えた（validate / lock / plan / apply）。
> P2 の E001（workflow）は workspace.md で「1 instance deployable の範囲外」として**意図的に後回し**、
> E002/E003 は別途。まずは増えたコマンド面のUXを底上げする Q002 を行う（小さく確実な改善）。

**概要（ゴール）:** 新コマンド（manifest validate / lock / plan / apply）に**使用例**を出し、
`juice <cmd> -h` と `juice -h` から「何をどう叩くか」が分かるようにする。宣言ライフサイクル
（validate → lock → plan → apply）の典型フローも提示する。

**タスク（チェックリスト）:**
- [ ] argparse の各サブパーサに `epilog`（例: `juice lock -f juice.yaml -o juice.lock`）や説明を追加。
- [ ] トップレベル parser に典型ワークフロー（validate → lock → plan → apply）の epilog を追加
      （`RawDescriptionHelpFormatter` を使い整形を保つ）。
- [ ] tests/test_cli.py: `build_parser().format_help()` や各サブの help に例文字列が含まれることを確認。
- [ ] docs: README か build.md に宣言系コマンドの使い方を一節追加（既存 build.md は bundle 系中心なので補う）。

**完了条件:** 主要コマンドの `-h` に使用例が出る。`make check` 緑（help 検証テスト含む）。

**留意点:** 出力整形（help 文）は壊れやすいので、テストは「例文字列が含まれる」程度の緩い一致にする。
振る舞い（dispatch）は変えない。
