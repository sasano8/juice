...
juice.yaml ──lock──▶ 解決＋整合 ──apply──▶ registries/（生成物）
                                              │
              mcp_bundled ──bundle/build──▶ docker image
                                              │
...
- **レイヤの分離:** tool / skill / subagent / mcp_bundled / instance / workflow / schedule。
...
（サンプル `weather-bot`）。`bundle → build → run` で起動でき、api / ui / mcp_server の各モードを持つ。
...
- 概念・レイヤ関係（mcp_server / mcp_bundled / subagent / skill / tool / workflow / schedule）:
