...
juice の実用ガイド。`bundle.yml`（bundle の定義）から **init → bundle → build → run** で
...
## bundle.yml（bundle の定義）

`namespaces/<namespace>/bundles/<name>/bundle.yml` が単一のソース（instance の index.yml に相当）。
...
image: juice/mcp_weather-bot     # build するイメージ名（タグなし）
version: 0.0.1
...
juice bundle init mcp_weather-bot [-n default] [--clean]

# vendoring ＋ LangGraph 一式生成（vendor/ を作る）
juice bundle bundle mcp_weather-bot [-n default]

# docker イメージビルド（実行する）
juice bundle build mcp_weather-bot [-n default] [-t juice/mcp_weather-bot:latest]

# 起動。mode = api / ui / mcp_server（既定 api）
juice bundle run mcp_weather-bot [api|ui|mcp_server] \
...
```
namespaces/<namespace>/          # 最上位は namespaces/。各 <layer> が 1 つの registry
  tools/<name>/index.md          # + server.py 等（MCP server 単位）
  skills/<name>/SKILL.md
  subagents/<name>/index.md
  bundles/<name>/bundle.yml       # bundle レイヤ（dir は複数形 bundles）+ vendor/（生成物）
```
...
`bundle.yml`（bundle 単位）とは別に、`juice.yaml`（宣言的ワークスペース manifest）から
