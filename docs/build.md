...
`registries/namespaces/<namespace>/bundles/<name>/bundle.yml` が単一のソース（instance の index.yml に相当）。
...
```
registries/namespaces/<namespace>/
  tools/<name>/index.md        # + server.py 等（MCP server 単位）
  skills/<name>/SKILL.md
  subagents/<name>/index.md
  bundles/<name>/bundle.yml     # mcp_bundled レイヤ（dir は複数形 bundles）+ vendor/（生成物）
```
