# build：mcp_server → actor → instance のビルド手順（仮）
...
「素の MCP server」から「起動中の actor instance」までを、パッケージマネージャー的な
ライフサイクルで一直線に通す。コンテナの `build → push → run` に対応させる。

```
mcp_server        … デプロイ可能な成果物（= 1 capability の提供元）          [build / deploy]
   └─ actor       … subagent / skill / mcp_server(tool) を集約するファサード   [compose / build]
        └─ instance … actor に secret を注入した起動可能な実個体               [instantiate / start]
```

現状は **1 mcp_server = 1 tool**。actor はその上の **集約層**で、複数の mcp_server（tool）＋
subagent ＋ skill を 1 つの実行可能エージェントに束ねる。逆に言うと、actor として instance 化
...
```
instance → workflow → actor → subagent → skill → tool → mcp_server
```
...
# 1-d. namespace へデプロイ（= push 相当）。actor から参照可能になる。
juice mcp_server deploy weather --namespace default

juice mcp_server list
```

> ここまでで「actor に依存される前に、単体でデプロイ可能な成果物」が出来る。
> mcp_server 単体を別の actor から再利用できる（集約層との分離点）。
...
juice workflow add-step morning-brief --actor weather-bot --input '{"city":"Tokyo"}'
...
actor に取り込む前に mcp_server を常設デプロイし、actor は**参照だけ**するパターン。
リモート/共有 server（複数 actor が同じ server を使う）に向く。

```bash
juice mcp_server build weather
juice mcp_server deploy weather --namespace default      # 常設

# actor 側は実体を持たず参照に徹する
juice actor bind-tool weather-bot weather --ref mcp_server:weather@default
juice actor build weather-bot
```

- **取り込み型（手順 3）** … actor が server 定義ごと抱える。可搬・自己完結。
- **参照型（別案）** … server は共有資産、actor は薄い結線。再利用・集中管理向き。
...
| actor 作成 | `juice actor new <name> --subagent <sa>` |
| actor 結線 | `juice actor add-skill / bind-tool <actor> ...` |
| actor 確定 | `juice actor build <name>` |
| instance 化 | `juice actor instantiate <actor> --name <inst> --set K=env:V` |
