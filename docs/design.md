...
```
workflow          … 複数の mcp_bundled を束ねる実行スケジューラ
   └─ mcp_bundled       … subagent / skill / tool を結線・具象化したファサード（実行可能エージェント）
...
| **mcp_bundled** | subagent / skill / tool を結線し、単一の実行可能エージェントとして見せる **ファサード（合成物）テンプレート** | イメージ＋起動設定 |
| **workflow** | 複数の mcp_bundled を束ねる実行スケジューラ | docker-compose / DAG |

### tool は subagent と mcp_bundled の両方に現れるが役割が違う
...
| **mcp_bundled** | 「**実体を渡し、起動可能にする**」= 提供と結線（facade） |

例: subagent は「github を使ってよい」と宣言するだけ。mcp_bundled が「これが github の MCP server」と実体を結線する。
...
- **テンプレート** … `mcp_bundled/` 等。依存を宣言した雛形。可搬・再利用可能。
- **instance** … `instances/`。テンプレート（主に mcp_bundled）に具象値を注入した実個体。`docker ps` で並ぶイメージ。
...
│   ├── mcp_bundled/        …   mcp_bundled テンプレート（subagent / skill / tool の合成）
│   ├── workflows/     …   workflow テンプレート
│   └── instances/     …   具象（mcp_bundled の実個体 / docker ps 相当）
...
| 種別 | エントリファイル | 形式 | 理由 |
|------|----------------|------|------|
| tool / subagent / mcp_bundled / workflow | `<name>/index.md` | YAML frontmatter + 本文 | 本文に散文（prompt 等）を持ちうる |
| skill | `<name>/SKILL.md` | YAML frontmatter + 本文 | Claude Code 標準に準拠 |
| instance | `<name>/index.yml` | 純 YAML | 具象パラメータのみで散文不要 |

frontmatter／YAML の先頭は `kind:`（`tool` / `skill` / `subagent` / `mcp_bundled` / `workflow` / `instance`）で型を識別する。
...
| **mcp_bundled** | **独自スキーマ（合成・ファサード層）** | subagent を参照し、skill / tool の実体を結線する。標準では表現できないため独自定義 |
...
### subagent と mcp_bundled の責務分離

「subagent 形式を拡張して mcp_bundled を表現する」のではなく、不一致（具象化・skill 依存宣言）を
拡張で埋めず **層を分けて解決**する。

- **subagent** … 標準フォーマットのまま。拡張しない → 他ランタイムへもそのまま持ち出せる。
- **mcp_bundled** … subagent に足りない点を上位層で補う独自スキーマ：
...
- **依存解決** … `workflow → mcp_bundled → (subagent, skill, tool)` の参照を名前で引き当て・検証
- **具象化** … `${WEATHER_API_KEY}` 等の env / secret を instance に注入
- **versioning** … 例: `mcp_bundled@1.2` が `subagent@>=2.0` を要求、等
- **依存方向の保証** … `subagent → (model, tool-allowlist)`、`mcp_bundled → (subagent, skill, tool)`、`workflow → mcp_bundled`、`instance → mcp_bundled`
