...
| 階層 | 役割 | 例 | 物理対応 |
|------|------|----|----------|
| レイヤー1 | namespace（リソース空間の区画） | `default` | `registries/namespaces/<namespace>/` |
| レイヤー2 | kind / layer（成果物の種類） | `tools` / `skills` / `bundles` / … | `…/<namespace>/<layer>/` |
| 成果物ディレクトリ | **name（主役の資産＝1パッケージ=1ディレクトリ）** | `weather` | `…/<layer>/<name>/` |

物理レイアウトは `registries/namespaces/<namespace>/<layer>/<name>/` で、**Kubernetes の API パス
`/namespaces/<ns>/<kind>/<name>`（namespace × kind × name）と同型**（[config.py](../src/core/config.py) 冒頭コメント）。
namespace も layer も**複数形の容器ディレクトリ**（`namespaces/` / `tools/` / `bundles/` …）の下に実体が並ぶ規約。

```
registries/                      ← bucket（ストレージの入れ物）
└── namespaces/                  ← namespace の容器（複数形）
    └── default/                 ← namespace（レイヤー1）
        ├── tools/               ← layer / kind（レイヤー2・複数形）
        │   └── weather/         ← 成果物ディレクトリ（name・主役）
        │       └── index.md     ← entry file
        ├── skills/ └ report-weather/SKILL.md
        ├── subagents/ └ forecaster/index.md
        ├── bundles/             ← mcp_bundled レイヤの実ディレクトリ（旧 mcp_bundled）
        │   └── weather-bot/ ├ bundle.yml  └ vendor/ …（生成物）
        ├── workflows/ └ weather-service/index.md
        ├── schedules/ └ morning-brief/index.md
        └── instances/ └ tokyo-weather-bot/index.yml
```

> **命名規約:** レイヤキー（CLI コマンド・manifest 上の概念名）は `mcp_bundled` のままだが、registry 上の
> ディレクトリ名は他レイヤと揃えて複数形 **`bundles`**（`config.LAYERS["mcp_bundled"] = "bundles"`）。
