...
`bundle.yml` が bundle の **定義 + ビルド宣言**（instance の index.yml に相当する単一の
...
VENDOR_DIR = "vendor"
# 表示用パスの接頭辞。レイヤキー bundle の実ディレクトリ名（= bundles）。
_BUNDLE_DIR = LAYERS["bundle"]
...
BUNDLE_YML_TEMPLATE = """\
# bundle.yml — bundle の定義 + ビルド宣言（juice bundle init が生成。編集可）
apiVersion: juice/v1
kind: bundle
name: {name}
namespace: {namespace}
...
DOCKERFILE = """\
# generated by juice — `juice bundle bundle` で再生成（手編集しない）
...
def _spec(registries: RegistryArray, name: str) -> dict:
    return yaml.safe_load(registries.read("bundle", name, SPEC_FILE)) or {}
...
def _vendor(registries: RegistryArray, name: str) -> list[str]:
    ...
    for layer, dep in _deps(spec, include):
        for rel in registries.list_files(layer, dep):  # パッケージ配下の全ファイル
            ...
            registries.write("bundle", name, out, raw)
...
def init(registries: RegistryArray, name: str, clean: bool = False) -> dict:
    ...
    exists = registries.exists("bundle", name, SPEC_FILE)
    if not clean and exists:
        raise FileExistsError(
            f"already initialized: {_BUNDLE_DIR}/{name}/{SPEC_FILE} (use --clean to clean & reinitialize)"
        )
    if clean:
        spec_text = registries.read("bundle", name, SPEC_FILE) if exists else None
        registries.remove("bundle", name, "")
        if spec_text is None:
            spec_text = BUNDLE_YML_TEMPLATE.format(namespace=registries.namespace, name=name)
        registries.write("bundle", name, SPEC_FILE, spec_text)
    else:
        registries.write(
            "bundle",
            name,
    ...
    return {"kind": "init", "bundle": name, "spec": f"{_BUNDLE_DIR}/{name}/{SPEC_FILE}"}
...
def bundle(registries: RegistryArray, name: str) -> dict:
    ...
    for fname, text in (
        ...
        registries.write("bundle", name, f"{VENDOR_DIR}/{fname}", text)
        generated.append(f"{_BUNDLE_DIR}/{name}/{VENDOR_DIR}/{fname}")

    return {"kind": "bundle", "bundle": name, "vendored": vendored, "generated": generated}
...
def build(registries: RegistryArray, name: str, image: str | None = None) -> dict:
    ...
    context = registries.location("bundle", name, VENDOR_DIR)
    return {
        "kind": "build",
        "bundle": name,
        "image": tag,
