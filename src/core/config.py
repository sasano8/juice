...
リソース空間の区画（既定 `default`）、`path` がその中の 1 つの場所（local なら `bundles` 等）を
指す。物理レイアウトは `{bucket}/{NAMESPACE_CONTAINER}/{namespace}/{layer}/{name}/` で、
Kubernetes の API パス `/namespaces/<ns>/<kind>/<name>`（namespace × kind × name）と同型。
デフォルトや overrides の解決は factory が担い、ここには解決済みの値だけを持たせる。
複数レイヤを束ねるのは RegistryArray の役割。
"""
...
class Config:
    ...
    path: str  # namespace 内の 1 ロケーション。local なら "bundles" 等
