...
- **target は pluggable**。最初の実装は docker-compose（`compose`）。k8s manifest は将来の target。
...
def _dump(data: dict) -> str:
    """デプロイ成果物を決定的に直列化する（YAML＋生成物ヘッダ）。"""
    body = yaml.safe_dump(data, sort_keys=False, allow_unicode=True, default_flow_style=False)
    return _HEADER + body


# target 名 -> (builder, 出力ファイル名)。新 target（k8s 等）はここに足す。
_TARGETS: dict[str, tuple] = {
    "compose": (build_compose, "docker-compose.yml"),
}
...
def build_deployment(
    ...
    """(出力ファイル名, テキスト) を返す。target は pluggable（現状 `compose` のみ）。"""
    if target not in _TARGETS:
        raise ValueError(f"未対応の target: {target}（対応: {', '.join(_TARGETS)}）")
    builder, filename = _TARGETS[target]
    return filename, _dump(builder(manifest, workflow))
