...
from juice_storage import Storage

from .config import ENTRY_FILES, Config, namespace_root
...
class Registry:
    ...
    def location(self, name: str, entry: str = "") -> str:
        ...
        ns = namespace_root(self.config.bucket, self.config.namespace)
        base = f"{ns}/{self.config.path}/{name}"
